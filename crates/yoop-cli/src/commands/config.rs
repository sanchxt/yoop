...
pub async fn run(args: ConfigArgs) -> Result<()> {
    let mut config = yoop_core::config::Config::load()?;
    ...
    match args.action {
        ...
        ConfigAction::Path => {
            println!("{}", yoop_core::config::Config::config_path().display());
        }
...
fn get_config_value(config: &yoop_core::config::Config, key: &str) -> Option<String> {
    match key {
        // general
    ...
    match key {
        ...
        "trust.default_level" => {
            config.trust.default_level = match value.to_lowercase().as_str() {
                ...
                "ask_each_time" | "ask" => yoop_core::config::TrustLevel::AskEachTime,
